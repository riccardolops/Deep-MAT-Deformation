...
class Heart(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', transform=None):
        ...
        with open(os.path.join(base_dir, 'dataset_new.json')) as dataset_file:
            json_dataset = json.load(dataset_file)
        self.sample_list = []
        if split == 'train':
            self.dataset = json_dataset
        elif split == 'val':
            self.dataset = json_dataset[-10:]
    ...
    def __getitem__(self, idx):
        ...
        image = nib.load(self._base_dir / image_name['image']).get_fdata()
        label = nib.load(self._base_dir / image_name['label']).get_fdata()
